...
const EXPECTED_VALUES: Record<Letter, number> = {
...
describe('LETTER_VALUES', () => {
  it('should contain values for all 26 letters', () => {
    ...
    for (let i = 0; i < 26; i++) {
      const letter = String.fromCharCode(65 + i) as Letter // A=65, B=66, etc.
  ...
  it('should have correct Scrabble point values', () => {
    Object.entries(EXPECTED_VALUES).forEach(([letter, expectedValue]) => {
      expect(LETTER_VALUES[letter as Letter]).toBe(expectedValue)
    })
  })
