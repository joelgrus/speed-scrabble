...
describe('buildBag', () => {
    ...
    it('should shuffle tiles thoroughly', () => {
        ...
        let expectedCount = EXPECTED_DISTRIBUTION[expectedLetter];
        let currentCount = 0;
        for (const tile of bag) {
            ...
            if (currentCount === expectedCount) {
                // Move to next letter
                const letters = Object.keys(EXPECTED_DISTRIBUTION);
                const nextIndex = letters.indexOf(expectedLetter) + 1;
                if (nextIndex < letters.length) {
                    expectedLetter = letters[nextIndex];
                    expectedCount = EXPECTED_DISTRIBUTION[expectedLetter];
                    currentCount = 0;
                }
            }
        }
