...
export interface GameResult {
  username: string; // Player username/userid
  date: string; // Date in YYYY-MM-DD format for daily puzzles
  width: number;
  height: number;
  grid: string; // Row-by-row string with letters and dots for empty cells
 ...
 */
function compressGrid(grid: string): Uint8Array {
  const result: number[] = [];
  let i = 0;
 ...
 */
function decompressGrid(compressed: Uint8Array): string {
  let result = '';
  let i = 0;
 ...
 */
export function decodeGameResult(encoded: string): GameResult {
  try {
    // Decode base64
 ...
 */
export function createShareUrl(result: GameResult, baseUrl: string = ''): string {
 ...
 */
export function parseShareUrl(url: string): GameResult {
