...
export function set(board, t) {
    board[key(t.x, t.y)] = t;
}
export function del(board, x, y) {
    delete board[key(x, y)];
}
function collectWord(board, x, y, dx, dy) {
    // move to start of the run
    while (get(board, x - dx, y - dy)) {
        x -= dx;
        y -= dy;
    }
    const letters = [];
...
export function isConnected(board) {
