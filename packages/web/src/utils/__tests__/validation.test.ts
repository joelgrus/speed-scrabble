...
describe('sanitizeForDisplay', () => {
  ...
  it('should handle complex HTML injection attempts', () => {
    ...
    expect(sanitizeForDisplay(malicious)).toBe('&lt;img src=&quot;x&quot; onerror=&quot;alert(1)&quot;&gt;')
  })
})
...
describe('safeArraySplice', () => {
  ...
  it('should throw error for non-array input', () => {
    expect(() => safeArraySplice('not-array' as unknown as never[], 0, 1)).toThrow('First argument must be an array')
  })

  it('should throw error for non-integer indices', () => {
    const arr = [1, 2, 3]
    expect(() => safeArraySplice(arr, 1.5, 1)).toThrow('Start index must be an integer')
    expect(() => safeArraySplice(arr, 1, 1.5)).toThrow('Delete count must be a non-negative integer')
  })

  it('should throw error for negative delete count', () => {
    const arr = [1, 2, 3]
    expect(() => safeArraySplice(arr, 1, -1)).toThrow('Delete count must be a non-negative integer')
  })
})
...
describe('validateGameState', () => {
  const validState = {
    rack: [{ id: 't1', letter: 'A' }, { id: 't2', letter: 'B' }],
    bag: [{ id: 't3', letter: 'C' }],
    board: {
